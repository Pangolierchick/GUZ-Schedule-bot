...
def get_today_schedule_handler(message):
    ...
    if group is None:
        bot.reply_to(message, 'Вы не зарегестрированы. Напишите /register, чтобы зарегестрироваться')
        return
...
def register_user(message):
    ...
    sent = bot.send_message(message.chat.id, 'Выберите вашу группу: ', reply_markup=markup)
    bot.register_next_step_handler(sent, insert_user_into_base)

def insert_user_into_base(message):
    dbase.set_user(message.from_user.id, message.from_user.username, message.text)
...
def morning_send_schedule():
    ...
    for i in users:
        ...
        bot.send_message(i[1], f'Доброе утро, {i[2]}, твое расписание на сегодня:\n {sch}')
        time.sleep(2)
